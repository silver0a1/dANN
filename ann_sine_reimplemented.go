		...
		for _, d := range data {
			...
			for l := len(ann.layers) - 2; l >= 0; l-- {
				...
				for i, neuron := range currentLayer.neurons {
					...
					neuron.delta = sum * reluDerivative(neuron.output)
				}
			}
			...
			for _, layer := range ann.layers {
				layerOutputs := make([]float64, len(layer.neurons))
				for i, neuron := range layer.neurons {
					for j, input := range inputs {
						neuron.weights[j] += learningRate * neuron.delta * input
					}
					neuron.bias += learningRate * neuron.delta
					layerOutputs[i] = neuron.output
				}
				inputs = layerOutputs
			}
		}
...
func main() {
	...
	// 2 inputs (x, y), two hidden layers (64, 32), 1 output (sin(x*y))
	ann := NewANN([]int{2, 64, 32, 1})
	fmt.Println("--- Training Re-implemented ANN on Sine Approximation problem ---")
