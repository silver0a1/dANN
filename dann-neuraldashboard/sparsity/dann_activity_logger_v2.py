...
def run_one_epoch(data, lr, layers_data):
    ...
    for idx in indices:
        ...
        # Forward pass
        l0_out = np.tanh(l0_w.reshape(l0_w.shape[0] * l0_w.shape[1], l0_w.shape[2]) @ x).reshape(l0_w.shape[0], l0_w.shape[1]) + l0_b
        l0_comp_out = np.tanh(l0_out)
        l0_soma = np.sum(l0_comp_out * l0_soma_w, axis=1) + l0_soma_b
        l0_activated = 1 / (1 + np.exp(-l0_soma))

        l1_out = np.tanh(l1_w.reshape(l1_w.shape[0] * l1_w.shape[1], l1_w.shape[2]) @ l0_activated).reshape(l1_w.shape[0], l1_w.shape[1]) + l1_b
        l1_comp_out = np.tanh(l1_out)
        l1_soma = np.sum(l1_comp_out * l1_soma_w, axis=1) + l1_soma_b
        l1_activated = 1 / (1 + np.exp(-l1_soma))
        ...
        # Backward pass
        d_l1_activated = err * (l1_activated * (1 - l1_activated))
        d_l1_soma = d_l1_activated
        d_l1_soma_w = d_l1_soma.reshape(-1, 1) * l1_comp_out
        d_l1_comp_out = d_l1_soma.reshape(-1, 1) * l1_soma_w * (1 - l1_comp_out**2)

        d_l1_out = d_l1_comp_out
        d_l1_b = np.sum(d_l1_out, axis=0)
        d_l1_w = np.outer(d_l1_out.flatten(), l0_activated).reshape(l1_w.shape)

        d_l0_activated = (l1_w.reshape(l1_w.shape[0] * l1_w.shape[1], l1_w.shape[2]).T @ d_l1_out.flatten()).T
        d_l0_soma = d_l0_activated * (l0_activated * (1 - l0_activated))
        d_l0_soma_w = d_l0_soma.reshape(-1, 1) * l0_comp_out
        d_l0_comp_out = d_l0_soma.reshape(-1, 1) * l0_soma_w * (1 - l0_comp_out**2)

        d_l0_out = d_l0_comp_out
        d_l0_b = np.sum(d_l0_out, axis=0)
        d_l0_w = np.outer(d_l0_out.flatten(), x).reshape(l0_w.shape)

        # Update weights
        l1_w += lr * d_l1_w
        l1_b += lr * d_l1_b
        l1_soma_w += lr * d_l1_soma_w
        l0_w += lr * d_l0_w
        l0_b += lr * d_l0_b
        l0_soma_w += lr * d_l0_soma_w

    layers_data = (l0_w, l0_b, l0_soma_w, l0_soma_b, l1_w, l1_b, l1_soma_w, l1_soma_b)
    ...
    def log_activity(self, epoch, data, log_dir):
        ...
        def save_and_plot(layer, comps, acts, prev_sparsity_attr):
            ...
            if prev_sparsity is not None:
                ...
                print(f"[Epoch {epoch}] D Layer {layer} Sparsity: {delta:.6f}")
