...
func main() {
	...
	splitIndex := int(float64(len(bostonData)) * 0.8)
	trainData := bostonData[:splitIndex]
	testData := bostonData[splitIndex:]

	// 13 inputs, 8 hidden neurons (example), 1 output
	ann := NewdANN([]int{13, 8, 1}, 8) 
	fmt.Println("--- Training dANN on Boston Housing problem ---")
	ann.Train(trainData, 5000, 0.01) // Increased epochs for regression
	fmt.Println("\n--- Training Complete ---")

	fmt.Println("\n--- Testing Trained dANN on Boston Housing ---")
	...
	fmt.Printf("\nFinal RMSE on Boston Housing: %.2f\n", rmse)
}