...
func loadBostonHousingData(filePath string) ([]TrainingData, *MinMaxScaler, *MinMaxScaler, error) {
...
func runCrossValidation(bostonData []TrainingData, targetScaler *MinMaxScaler, learningRate float64) float64 {
	kFold := 5
	var foldRMSEs []float64

	fmt.Printf("--- Starting %d-Fold Cross-Validation with Learning Rate: %f ---\n", kFold, learningRate)

	// Shuffle the data once before cross-validation
	rand.Shuffle(len(bostonData), func(i, j int) { bostonData[i], bostonData[j] = data[j], data[i] })
	...
	for i := 0; i < kFold; i++ {
		...
		ann.Train(trainData, 5000, learningRate) // Use the passed learning rate
	...
	fmt.Printf("\n--- Cross-Validation Complete for Learning Rate: %f ---\n", learningRate)
	fmt.Printf("Individual Fold RMSEs: %v\n", foldRMSEs)
...
func main() {
	...
	learningRates := []float64{0.1, 0.01, 0.001}

	fmt.Println("--- Starting Learning Rate Tuning ---")
	for _, lr := range learningRates {
		avgRMSE := runCrossValidation(bostonData, targetScaler, lr)
		fmt.Printf("Overall Average RMSE for Learning Rate %f: %.2f\n\n", lr, avgRMSE)
	}
	fmt.Println("--- Learning Rate Tuning Complete ---")
}