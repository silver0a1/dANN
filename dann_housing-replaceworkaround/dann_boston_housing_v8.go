...
// LogTransform applies log1p transformation
func (s *MinMaxScaler) LogTransform(value float64) float64 {
	return math.Log1p(value)
}

// InverseLogTransform applies inverse log1p transformation
func (s *MinMaxScaler) InverseLogTransform(value float64) float64 {
	return math.Exp(value) - 1
}

// StandardScaler stores mean and standard deviation for scaling
type StandardScaler struct {
	mean []float64
	std  []float64
}

// NewStandardScaler initializes the scaler with mean/std values from data
func NewStandardScaler(data [][]float64) *StandardScaler {
	numFeatures := len(data[0])
	...
	for i := 0; i < numFeatures; i++ {
		...
		sumSqDiff := 0.0
		for _, row := range data {
			sumSqDiff += math.Pow(row[i]-means[i], 2)
		}
		stds[i] = math.Sqrt(sumSqDiff / float64(len(data)))
		if stds[i] == 0 { // Avoid division by zero
			stds[i] = 1.0
		}
	}
	return &StandardScaler{mean: means, std: stds}
}

// Transform scales the input values using standardization
func (s *StandardScaler) Transform(inputs []float64) []float64 {
	scaledInputs := make([]float64, len(inputs))
	for i, val := range inputs {
		scaledInputs[i] = (val - s.mean[i]) / s.std[i]
	}
	return scaledInputs
}

// InverseTransform scales the output value back to original range using standardization
func (s *StandardScaler) InverseTransform(scaledValue float64, featureIndex int) float64 {
	return scaledValue*s.std[featureIndex] + s.mean[featureIndex]
}
...
func loadBostonHousingData(filePath string) ([]TrainingData, *StandardScaler, *MinMaxScaler, error) {
	...
	for i := range rawInputs {
		...
		// Apply log transform to target before scaling
		logTransformedTarget := targetScaler.LogTransform(rawTargets[i][0])
		scaledTarget := targetScaler.Transform([]float64{logTransformedTarget})[0] // Corrected: Pass as []float64
		data = append(data, TrainingData{inputs: scaledInputs, expected: []float64{scaledTarget}}) // Corrected: Wrap scaledTarget in a slice
	}
...
func main() {
	...
	bostonData, _, targetScaler, err := loadBostonHousingData("C:/run/TestGemCli/dANN/boston_housing.csv")
	if err != nil {
		log.Fatal(err)
	}
	...
	for i := 0; i < kFold; i++ {
		...
		for _, d := range testData {
			...
			// Inverse transform prediction and expected values
			predictionLogTransformed := targetScaler.InverseTransform(predictionScaled, 0)
			expectedLogTransformed := targetScaler.InverseTransform(expectedScaled, 0)

			predictionOriginal := targetScaler.InverseLogTransform(predictionLogTransformed)
			expectedOriginal := targetScaler.InverseLogTransform(expectedLogTransformed)

			totalSquaredError += math.Pow(expectedOriginal-predictionOriginal, 2)

			// fmt.Printf("Expected: %.2f, Predicted: %.2f (Scaled Expected: %.4f, Scaled Predicted: %.4f)\n", expectedOriginal, predictionOriginal, expectedScaled, predictionScaled)
		}
